...
var _EventMarker = require("react-timeseries-charts/lib/components/EventMarker");

var _EventMarker2 = _interopRequireDefault(_EventMarker);

var _util = require("react-timeseries-charts/lib/js/util");

var _styler = require("react-timeseries-charts/lib/js/styler");
      ...
      this.props.columns.forEach(function (column) {
        ...
        try {
          var _loop = function _loop() {
            ...
            var t = new Date(event.begin().getTime());
            var dt = new Date((event.get("endTime")));
            var value = event.get(column);
            var style = _this2.style(column, event);

            var dx = timeScale(t);
            var dy = yScale(value);
            var dxFar =  timeScale(dt);
       ...
       //     console.log(event.get("numberOfLocations") * 1 +"   " + event.get("numberOfSpecies"));
            points.push(_react2.default.createElement("rect", {
              key: column + "-" + key,
              x: dx,
              y: dy,
              width: dxFar-dx > 0 ? (dxFar-dx)+.5:0,
              height: event.get("width"),
              rx: 0,
              ry: 0,
              style: style,
...
ScatterChart.propTypes = {
   ...
   */
  highlight: _react2.default.PropTypes.shape({
    event: _react2.default.PropTypes.instanceOf(_pondjs.Event),
   ...
   */
  onMouseNear: _react2.default.PropTypes.func,
  /**
